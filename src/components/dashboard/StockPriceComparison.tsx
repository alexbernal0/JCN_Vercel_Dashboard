...
export default function StockPriceComparison({
  symbols,
}: StockPriceComparisonProps) {
  ...
  const chartData = useMemo(() => {
    ...
    for (const [symbol, prices] of Object.entries(data.data)) {
      ...
      const filteredPrices = arr.filter(
        (p) => p && new Date(p.date) >= cutoffDate,
      )
    ...
    return {
      tooltip: {
        ...
        formatter: (params: unknown) => {
          ...
          ;(
            p as { seriesName: string; value?: number | [string, number] }[]
          ).forEach((item) => {
    ...
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-6 text-center">
        <p className="text-red-600">
          Failed to load stock price data. Please try refreshing.
        </p>
      </div>
    )
  }
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      ...
      <div className="border-t border-gray-200 pt-6">
        ...
        <p className="mb-3 text-xs text-gray-500">
          Click a button to show only that period; Y-axis is return vs. period
          start (00.00%).
        </p>
